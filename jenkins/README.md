...
### Add master executor
Before running any job, it is needed to add an executor which will run a Jenkins job. First, it is required to add an executor on the master node - which is a server containing the Jenkins instance. This executor will lunch the job and then a pipeline will be executed on the node with a label matching the "LABEL" field in the job params.
...
- Installed and running a SSH server, open ssh port (22).
