...
# Region used for dax namespaces. The number represents region's socket ID, as seen by using command 'ipmctl show -region'.
DEV_DAX_R=0x0000
# Region used for fsdax namespaces. The number represents region's socket ID, as seen by using command 'ipmctl show -region'.
FS_DAX_R=0x0001
CREATE_DAX=false
CREATE_PMEM=false
MOUNT_POINT="/mnt/pmem0"
# Default size of namespaces.
SIZE=100G
...
ndctl list -X | jq -r '.[] | select(.mode=="devdax") | [.daxregion.devices[].chardev, "align:\
 "+(.align/1024|tostring+"k"), "size: "+(.size/1024/1024/1024|tostring+"G") ]'
