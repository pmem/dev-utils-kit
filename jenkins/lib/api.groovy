...
/* A list of git revisions used in a job. It should contain list of dicts (with 'url' and 'sha' keys) */
saved_git_revisions = []
...
/* Required to use top-level function inside a class method */
...
def run_bash_script(script_text, log_file = LOG_FILE, import_common_file = false, error_on_non_zero_rc = true) {
	/**
	 * 1. Separate log file for this current command exclusively, with a random name.
	 * 2. If requested - append to the given log file.
	 */
	...
	/* Capture script output from the file. */
...
/**
 * Add repository revision to saved revisions. This is used for example to send an e-mail with the test result.
...
/**
 * Return git revision string with all cloned repositories.
...
/**
 * Print OS and BRANCH params of the current job to Jenkins. Call script which collects system info and saves it to logs.
 */
def system_info() {
...
/**
 * Archive artifacts from the output directory.
 */
def archive_output() {
...
/**
 * Get the week number of date when current build has started.
