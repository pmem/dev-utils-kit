...
/* List of git revisions used in a job. It should contain list of dicts (with 'url' and 'sha' keys) */
saved_git_revisions = []
...
/**
 * Add repository revision to saved revisions. This is used for example to send e-mail with test result.
 * If requested repository (matching URL and SHA) is already known, it is not added for the second time.
...
def clone_repository(repo_url, branch, target_directory = '', keep_sha = true) {
	...
	def revision = checkout([$class: 'GitSCM', branches: [[name: "${branch}"]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: target_directory]], submoduleCfg: [], userRemoteConfigs: [[url: repo_url]]])
